...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["code"])) {
    ...
    if (isset($_SESSION["email"])) {
        ...
        // Update the user's email_otp column with the 2FA code
        $update_sql = "UPDATE user_submissions SET email_otp='$code' WHERE email='$email'";
        if ($conn->query($update_sql) === TRUE) {
            // Code stored successfully
            echo "Code stored successfully";
            exit();
        } else {
            // Error updating email_otp in the database
            echo "Error updating email_otp: " . $conn->error;
        }
    } else {
